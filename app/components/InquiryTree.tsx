  ...
  return (
    <section className="shell inquiry-explorer">
      <div className="inquiry-toolbar">
        <div className="mode-links" aria-label="Inquiry views"><a className="selected" href="/inquiry">Tree mode</a><a href="/inquiry/list">List mode</a></div>
      ...
      {searching && (
        <div className="tree-search-results" aria-live="polite">
          {match ? <><p>Claim found</p><div><a href={`/inquiry/list#claim-${match.id}`}><b>#{match.id}</b> {match.title}</a></div></> : <p>No claim with that number.</p>}
        </div>
      )}
      ...
      <div className="tree-wrap concentric-tree-wrap">
        ...
        <div className="concentric-tree" aria-label="Concentric Tree of Inquiry. Connections will be added later.">
          ...
          {specificClaims.map((claim, index) => { const position = point(index, specificClaims.length, 45); return show(claim.id) ? <a className="tree-claim-node tree-specific-node" href={`/inquiry/list#claim-${claim.id}`} key={claim.id} style={{ left: `${position.x}%`, top: `${position.y}%` }} aria-label={`Open claim ${claim.id}: ${claim.title}`}><span>#{claim.id}</span><b>{claim.title}</b></a> : null; })}
          {focusedClaims.map((claim, index) => { const position = point(index, focusedClaims.length, 32, -45); return show(claim.id) ? <a className="tree-claim-node" href={`/inquiry/list#claim-${claim.id}`} key={claim.id} style={{ left: `${position.x}%`, top: `${position.y}%` }} aria-label={`Open claim ${claim.id}: ${claim.title}`}><span>#{claim.id}</span><b>{claim.title}</b></a> : null; })}
      ...
      <div className="tree-bottom-actions"><p className="tree-philosophy"><b>Connections are intentionally blank.</b> The project’s administrators will add lines after deciding which claims support one another.</p><a className="button button-quiet" href="/qa-rules#rules">Read the discussion rules</a></div>
    </section>
  );
}
