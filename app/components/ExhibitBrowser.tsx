...
export function ExhibitBrowser() {
  ...
  return (
    <section className="shell exhibits-browser">
      ...
      {shown.length ? <div className="exhibit-list">{shown.map((exhibit) => <article className="exhibit-card" key={exhibit.no}><div className="exhibit-row"><span className="exhibit-number">Exhibit #{exhibit.no}<b>Template</b></span><div><h2>{exhibit.title}</h2><p>{exhibit.description}</p></div><div className="exhibit-action">{exhibit.href ? <a href={exhibit.href}>Open exhibit <span aria-hidden="true">↗</span></a> : <span className="exhibit-link-placeholder">[File or source link]</span>}</div></div><ExhibitComments exhibitNo={exhibit.no} exhibitTitle={exhibit.title} /></article>)}</div> : <div className="exhibits-placeholder" aria-live="polite"><span>No result</span><h2>No exhibit has that number.</h2><p>Search for Exhibit #1 to view the default template.</p></div>}
    </section>
  );
}
